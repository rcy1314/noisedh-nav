...
;(function($, window, undefined){
	...
	var resizeTicking2 = false;
	window.addEventListener('resize', function(){
		if (resizeTicking2) return;
		resizeTicking2 = true;
		window.requestAnimationFrame(function(){
			trigger_resizable();
			resizeTicking2 = false;
		});
	}, { passive: true });
})(jQuery, window);
...
function sidebar_menu_item_expand($li, $sub)
{
	...
	$sub.css({display: 'block', height: 'auto', visibility: 'hidden'});
	sub_height = $sub.outerHeight();
	$sub.css({height: 0, visibility: 'visible'});

	if (typeof TweenMax !== 'undefined' && TweenMax && typeof TweenMax.to === 'function')
	{
		TweenMax.to($sub, sm_duration, {css: {height: sub_height}, onUpdate: ps_update, onComplete: function()
		{
			$li.data('is-busy', false).addClass('opened');
			$sub.attr('style', '').show();
	...
	{
		$sub.attr('style', '').hide().stop(true, true).slideDown(200, function() {
			$li.data('is-busy', false).addClass('opened');
			$sub_items.removeClass('hidden-item');
			ps_update(true);
		});
